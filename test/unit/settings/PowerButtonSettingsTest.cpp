...
// Inline the Settings enums and getPowerButtonDuration from SagaTaluSettings.h
namespace sagatalu {
struct Settings {
...
int main() {
  ...
  runner.expectEq(uint8_t(0), uint8_t(sagatalu::Settings::PowerIgnore), "PowerIgnore == 0");
  runner.expectEq(uint8_t(1), uint8_t(sagatalu::Settings::PowerSleep), "PowerSleep == 1");
  runner.expectEq(uint8_t(2), uint8_t(sagatalu::Settings::PowerPageTurn), "PowerPageTurn == 2");

  // === Default setting ===
  {
    sagatalu::Settings settings;
    runner.expectEq(uint8_t(sagatalu::Settings::PowerIgnore), settings.shortPwrBtn,
  ...
  {
    sagatalu::Settings settings;
    settings.shortPwrBtn = sagatalu::Settings::PowerIgnore;
  ...
  {
    sagatalu::Settings settings;
    settings.shortPwrBtn = sagatalu::Settings::PowerSleep;
  ...
  {
    sagatalu::Settings settings;
    settings.shortPwrBtn = sagatalu::Settings::PowerPageTurn;
  ...
  // instead of checking rtcPowerButtonDurationMs <= 10
  {
    ...
    settings.shortPwrBtn = sagatalu::Settings::PowerSleep;
    runner.expectTrue(settings.shortPwrBtn == sagatalu::Settings::PowerSleep,
    ...
    settings.shortPwrBtn = sagatalu::Settings::PowerIgnore;
    runner.expectFalse(settings.shortPwrBtn == sagatalu::Settings::PowerSleep,
