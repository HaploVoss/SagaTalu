...
void fileEntry(const GfxRenderer& r, const Theme& t, int y, const char* name, bool isDir, bool selected,
  ...
  const int maxTextW = w - 2 * t.itemPaddingX - tagTextW;
  ...
  if (tagText[0] != '\0' && !showConvert) {
    // Progress and/or hint label — right-aligned text
    const int progX = x + w - t.itemPaddingX - r.getTextWidth(t.uiFontId, tagText);
    const int progY = y + (h - r.getLineHeight(t.uiFontId)) / 2;
    r.drawText(t.uiFontId, progX, progY, tagText,
               selected ? t.selectionTextBlack : t.secondaryTextBlack);
  } else if (showConvert) {
    const int tagW = r.getTextWidth(t.uiFontId, tagText) + 8;
    const int tagH = r.getLineHeight(t.uiFontId) + 2;
    const int tagX = x + w - t.itemPaddingX - tagW;
    const int tagY = y + (h - tagH) / 2;
    const int tagTxtY = tagY + 1;

    // Draw a small bordered badge
    r.drawRect(tagX, tagY, tagW, tagH, selected ? t.selectionTextBlack : t.secondaryTextBlack);
    r.drawText(t.uiFontId, tagX + 4, tagTxtY, tagText,
               selected ? t.selectionTextBlack : t.secondaryTextBlack);
  }
}
...
void twoColumnRow(const GfxRenderer& r, const Theme& t, int y, const char* label, const char* value) {
  ...
  r.drawText(t.menuFontId, labelX, y, label, t.primaryTextBlack);
  r.drawText(t.menuFontId, valueX, y, value, t.secondaryTextBlack);
}
