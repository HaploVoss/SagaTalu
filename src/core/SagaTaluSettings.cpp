...
Result<void> Settings::save(drivers::Storage& storage) const {
  ...
  storage.mkdir(SAGATALU_DIR);
  storage.mkdir(SAGATALU_CACHE_DIR);

  FsFile outputFile;
  auto result = storage.openWrite(SAGATALU_SETTINGS_FILE, outputFile);
  if (!result.ok()) {
    return result;
  }
...
Result<void> Settings::load(drivers::Storage& storage) {
  FsFile inputFile;
  auto result = storage.openRead(SAGATALU_SETTINGS_FILE, inputFile);
  if (!result.ok()) {
    return result;
  }
  ...
  if (magic != SETTINGS_MAGIC) {
    ...
    inputFile.close();
    storage.remove(SAGATALU_SETTINGS_FILE);
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    inputFile.read(reinterpret_cast<uint8_t*>(bleKeyboard), sizeof(bleKeyboard));
    bleKeyboard[sizeof(bleKeyboard) - 1] = '\0';
    if (++settingsRead >= fileSettingsCount) break;
...
bool Settings::saveToFile() const {
  ...
  FsFile outputFile;
  if (!SdMan.openFileForWrite("SET", SAGATALU_SETTINGS_FILE, outputFile)) {
    return false;
  }
...
bool Settings::loadFromFile() {
  FsFile inputFile;
  if (!SdMan.openFileForRead("SET", SAGATALU_SETTINGS_FILE, inputFile)) {
    return false;
  }
  ...
  if (magic != SETTINGS_MAGIC) {
    Serial.printf("[%lu] [SET] Invalid settings file (wrong magic 0x%08X), deleting\n", millis(), magic);
    inputFile.close();
    SdMan.remove(SAGATALU_SETTINGS_FILE);
    return false;
  }
