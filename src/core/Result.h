...
#define TRY(expr)                                                                                            \
  do {                                                                                                       \
    ...
    if (!TRY_PASTE(_try_res_, __LINE__).ok()) return ::sagatalu::ErrVoid(TRY_PASTE(_try_res_, __LINE__).err); \
  } while (0)

#define TRY_VAL(var, expr)                                               \
  auto _result_##var = (expr);                                           \
  if (!_result_##var.ok()) return ::sagatalu::ErrVoid(_result_##var.err); \
