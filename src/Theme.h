...
struct Theme {
  ...
  uint8_t itemHeight;       // File browser row height (compact)
  uint8_t itemSpacing;
  uint8_t itemPaddingX;      // Horizontal padding inside items
  uint8_t itemValuePadding;  // Right padding for values
  uint8_t menuItemHeight;    // Menu/settings row height (spacious)

  // Font IDs
  int uiFontId;
  int smallFontId;
  int menuFontId;            // Menu/settings font (larger than uiFontId)
